...
export const getMyAddressById = async (req, res, next) => {
  ...
  try {
    ...
    if (!address) {
      return next(
        new AppError("Endereço não encontrado ou não pertence a você.", 404)
      );
    }
    ...
    if (err.name === "CastError") {
      return next(
        new AppError(`ID de endereço inválido: ${req.params.id}`, 400)
      );
    }
    next(err);
  }
...
export const updateMyAddress = async (req, res, next) => {
  ...
  try {
    ...
    if (!address) {
      return next(
        new AppError(
          "Endereço não encontrado ou não pertence a você para atualização.",
          404
        )
      );
    }
...
export const deleteMyAddress = async (req, res, next) => {
  ...
  try {
    ...
    if (!address) {
      return next(
        new AppError(
          "Endereço não encontrado ou não pertence a você para deleção.",
          404
        )
      );
    }
