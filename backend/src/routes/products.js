...
const updateProductValidationRules = [
    ...
    body('description', 'Descrição inválida').optional({ nullable: true, checkFalsy: true }).trim(),
    body('stock', 'Estoque inválido').optional({ nullable: true }).isInt({ min: 0 }).toInt()
];
...
router.put('/:id', authenticate, isAdmin, mongoIdValidation('id', 'ID de produto inválido'), upload.single('image'), updateProductValidationRules, updateProduct);
