...
describe('/api/reviews', () => {
    ...
    describe('GET /product/:productId', () => {
        ...
        it('deve retornar a lista de avaliações de um produto', async () => {
            ...
            // Verifica se os IDs estão lá
            const reviewIds = res.body.data.reviews.map(r => r._id.toString());
            expect(reviewIds).toEqual(expect.arrayContaining([review1._id.toString(), review2._id.toString()]));
        });
        ...
        it('deve suportar paginação (page)', async () => {
            ...
            // Verifica se o ID retornado é diferente do primeiro (ordem createdAt DESC)
            expect(res.body.data.reviews[0]._id.toString()).not.toBe(review2._id.toString());
        });
